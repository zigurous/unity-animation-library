  ...
  function collapsibleSections() {
    ...
    for (i = 0; i < collapsibles.length; i++) {
      if (collapsibles[i].children.length > 6) {
        ...
        console.log(
          "Found show-bottom " +
            collapsibles[i].classList.contains("show-bottom")
        );
        if (collapsibles[i].classList.contains("show-bottom")) {
          for (
            var j = collapsibles[i].children.length - 1;
            j > collapsibles[i].children.length - 4;
            j--
          ) {
          ...
          .keyup(function () {
            ...
            if (query.length < 3) {
              ...
              $("#search-results>.search-list").text(
                format(UITexts._UI_SearchResultsFor, [query])
              );
            }
          })
          .off("keydown");
    ...
    function handleSearchResults(hits) {
      ...
      if (hits.length === 0) {
        $("#search-results>.sr-items").html(
          "<p>" + UITexts._UI_NoResultsFound + "</p>"
        );
      } else {
